...
export const api: IProductsApi = {
  ...
  async createProduct(
    ...
    category: string,
    price: number,
    imagePath: string = DefaultProduct
  ): Promise<void> {
    let products = LocalStorage.getItem(this._allProductsKey);
    let id: number;
    ...
    products.push({ id, name, description: '', category, price, imageUrl: imagePath });
